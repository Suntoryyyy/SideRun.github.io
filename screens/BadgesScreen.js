...
export default function BadgesScreen({ navigation }) {
  ...
  const loadUserData = async () => {
    try {
      ...
      if (currentUserStr) {
        ...
        if (data && !error) {
          const stats = {
            totalDistance: data.weeklyDistance || 0, // Using weekly as total
            totalRuns: data.totalRuns || 0,
            weeklyDistance: data.weeklyDistance || 0,
            weeklyRuns: data.totalRuns || 0, // Simplified map
          };
          setUserStats(stats);
  ...
  const checkBadgeUnlocks = async (stats, currentBadges, userId) => {
    const badgesToCheck = [
      {
        id: "first_5k",
        condition: stats.totalDistance >= 5,
        name: "First 5K Runner",
      },
      {
        id: "first_10k",
        condition: stats.totalDistance >= 10,
        name: "First 10K Runner",
      },
      {
        id: "marathoner",
        condition: stats.totalDistance >= 42.2,
        name: "Marathoner",
      },
      {
        id: "week_warrior",
        condition: stats.weeklyDistance >= 20,
        name: "Week Warrior",
      },
      {
        id: "consistent_runner",
        condition: stats.weeklyRuns >= 5,
        name: "Consistent Runner",
      },
      {
        id: "early_bird",
        condition: stats.weeklyRuns >= 3,
        name: "Early Bird",
      }, // Assuming morning runs
      {
        id: "social_butterfly",
        condition: stats.totalRuns >= 10,
        name: "Social Butterfly",
      },
      {
        id: "speed_demon",
        condition: stats.totalRuns >= 5,
        name: "Speed Demon",
      },
      {
        id: "night_runner",
        condition: stats.totalRuns >= 3,
        name: "Night Runner",
      },
    ];
      ...
      >
        ...
        <View style={styles.badgesContainer}>
          {filteredBadges.map((badge) => (
            <View
              key={badge.id}
              style={[styles.badgeCard, badge.unlocked && styles.unlockedBadge]}
            >
              <View style={styles.badgeHeader}>
                ...
                <View style={styles.badgeInfo}>
                  <Text
                    style={[
                      styles.badgeName,
                      badge.unlocked && styles.unlockedText,
                    ]}
                  >
                    {badge.name}
                  </Text>
                ...
                {badge.unlocked && (
                  <Text style={styles.unlockedIndicator}>✓</Text>
                )}
              </View>
              <Text
                style={[
